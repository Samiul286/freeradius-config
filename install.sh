...
#                                                                  #
#  GitHub: https://github.com/Samiul286/freeradius-config          #
#                                                                  #
####################################################################
"
sleep 5
...
# Initialise variables (FIX #1 #2: ensure they are always set)
pswd=""
dbname=""
dbuser=""
dbpass=""
...
# Install Webserver
read -r -p "Install Webserver (Apache2 & PHP 8.2) [Y/n]? " webserver
webserver=${webserver:-Y}
if [[ $webserver =~ ^[Yy]$ ]]; then
    echo ">>> Installing WebServer with PHP 8.2 <<<"

    # FIX #11: Install prerequisites for add-apt-repository (supports Ubuntu & Debian)
    sudo apt-get install -y software-properties-common lsb-release apt-transport-https ca-certificates

    # Add Ondrej PHP PPA (Ubuntu) or sury.org repo (Debian)
    if grep -qi "ubuntu" /etc/os-release; then
        sudo add-apt-repository ppa:ondrej/php -y
    else
        curl -sSLo /tmp/debsuryorg-archive-keyring.deb https://packages.sury.org/php/debsuryorg-archive-keyring.deb
        sudo dpkg -i /tmp/debsuryorg-archive-keyring.deb
        echo "deb https://packages.sury.org/php/ $(lsb_release -sc) main" | sudo tee /etc/apt/sources.list.d/php.list
    fi
    sudo apt-get update -y

    # Install Apache2 and PHP 8.2
    PHP_VER="8.2"
    sudo apt-get install -y apache2 \
        ...
        php${PHP_VER}-redis \
        php${PHP_VER}-apcu \
        libmcrypt-dev mcrypt make

    # FIX #12: Safely set PHP 8.2 as default using --install before --set
    sudo update-alternatives --install /usr/bin/php php /usr/bin/php${PHP_VER} 82
    sudo update-alternatives --set php /usr/bin/php${PHP_VER}
    sudo a2enmod php${PHP_VER}

    sleep 2
    echo ">>> Finished Installing WebServer with PHP ${PHP_VER} <<<"
    sleep 2
fi
...
1)
    ...
    # FIX #9: skip clone if already present
    if [ ! -d "phpnuxbill" ]; then
    ...
    # FIX #10: copy config BEFORE importing SQL
    ...
    sleep 2
    sudo rm -rf /var/www/html/phpnuxbill/install
    sleep 2
...
2)
    ...
    # FIX #3: only create symlinks if they don't already exist
    [ -L "$freeradius_config_dir/mods-enabled/sql" ] || \
        sudo ln -s "$freeradius_config_dir/mods-available/sql" "$freeradius_config_dir/mods-enabled/sql"

    # FIX #4: also link sqlcounter to mods-enabled
    [ -L "$freeradius_config_dir/mods-enabled/sqlcounter" ] || \
        sudo ln -s "$freeradius_config_dir/mods-available/sqlcounter" "$freeradius_config_dir/mods-enabled/sqlcounter"

    sudo mv $freeradius_config_dir/radiusd.conf $freeradius_config_dir/radiusd.conf.back 2>/dev/null || true
    sudo mv $freeradius_config_dir/sites-available/default $freeradius_config_dir/sites-available/default.back 2>/dev/null || true
    sudo cp ~/freeradius-config/config-files/default $freeradius_config_dir/sites-available/default
    sudo mv $freeradius_config_dir/sites-available/inner-tunnel $freeradius_config_dir/sites-available/inner-tunnel.back 2>/dev/null || true
    sudo cp ~/freeradius-config/config-files/inner-tunnel $freeradius_config_dir/sites-available/inner-tunnel
    sudo mv $freeradius_config_dir/mods-available/sql $freeradius_config_dir/mods-available/sql.back 2>/dev/null || true
    sudo cp ~/freeradius-config/config-files/sql $freeradius_config_dir/mods-available/sql
    sudo mv $freeradius_config_dir/mods-available/sqlcounter $freeradius_config_dir/mods-available/sqlcounter.back 2>/dev/null || true
    ...
    # FIX #5: only patch phpnuxbill config.php if it exists
    if [ -f /var/www/html/phpnuxbill/config.php ]; then
...
3)
    ...
    # Start FreeRADIUS
    # FIX #6: add missing chgrp for mods-config/sql/counter/mysql in case 3
    sudo chgrp -h freerad "$freeradius_config_dir/mods-enabled/sql"
    sudo chgrp -h freerad "$freeradius_config_dir/mods-enabled/sqlcounter"
    sudo chgrp -h freerad "$freeradius_config_dir/mods-available/sqlcounter"
    sudo chgrp -h freerad "$freeradius_config_dir/mods-available/sql"
...
# FIX #7: Restart the correct database service by name (not $MYSQL which = 'mysql' binary)
echo ">>> Restarting Database <<<"
sleep 2
db_service=$(echo "$db_server" | tr '[:upper:]' '[:lower:]')
sudo systemctl stop "$db_service"   2>/dev/null || true
...
sudo pkill -f freeradius 2>/dev/null || true
sudo systemctl start freeradius
sleep 2
...
- Password: admin
"
echo "LAMP (with $db_server), PHP 8.2, FreeRADIUS, and PHPNuxBill have been installed successfully!"
